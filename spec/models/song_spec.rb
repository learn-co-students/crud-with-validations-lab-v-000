...
RSpec.describe Song, type: :model do
  ...
  let(:missing_year) { valid_attributes.except(:year) }
  let(:unreleased) { missing_year.merge(released: false) }
  let(:future_release_year) { valid_attributes.merge(year: Date.today.year + 1) }
  ...
  it "is invalid without release year when released is true" do
    expect(Song.new(missing_year)).to be_invalid
  end
